...
class template extends commonClass {	
	...
	function printname($resume_id){
		...
		$out = '<div style="text-align:left; font-size:34px; font-family:Time new roman; color:#9a0101;" class="name">
	...
	function printaddress($resume_id){
		...
		$out = '<div style="text-align:left; border-top:2px solid #ff0000; font-family:Time new roman; font-size:15px; margin-top:1px;" class="address"> 
           '.$addr;
	...
	function print_all_sections($resume_id){
		...
		foreach($res_secs as $res_sec) {
			...
			<div style="font-size:23px; color:#9a0101;margin=0px;font-family:Time new roman; " class="section_title">'.$section['section_name'];
	...
	function print_single_value($resume_id, $section_id){
		...
		return '<div style="margin-left:1%;">'.$row[$field].'</div>';
	}
	function print_education($resume_id){
		...
		<div style="overflow:hidden;  margin-left:1%;" class="highlights">
			...
			<table style="width:100%">
					<tr>
						<td style="vertical-align:center; width:25%; text-align:left">
	...
	function print_experience($resume_id){
		...
		foreach($exp_value as $exp) {
			...
			$thisout .= '
			<div style=" overflow:hidden;margin-left:1%;margin-top:10px;" class="highlights">
				<div class="highlights_edit">
	...
	function print_work_history($resume_id){
		...
		foreach($work_value as $work) {
			...
			<div style=" overflow:hidden;margin-left:1%;margin-top:10px;" class="highlights">
				...
				<table style="width:100%">
				<tr>
					<td style="width:25%; vertical-align:center; text-align:left;">
						<div>'.$date_between.'</div>
					</td>
	...
	function print_executive_exp($resume_id){
		...
		<div style=" overflow:hidden;margin-left:1%;margin-top:10px;" class="highlights">
				...
				<table style="width:100%">
				<tr>
					<td style="width:25%; vertical-align:center; text-align:lelf;">'.$date_between.'</td>
					<td style="width:75%; vertical-align:top;">
	...
	function viewname($resume_id){
		$res = $this->load_single_resume($resume_id);		
		$out = '<div style="text-align:left; font-size:34px;color:#9a0101; font-family:Time new roman;" class="name">
           '.$res['first_name'].' '.$res['last_name'];
		$out .= '</div>';
		return $out;
	}
	...
	function viewaddress($resume_id){
		...
		$out = '<div style="text-align:left; border-top:2px solid #ff0000; font-size:15px; font-family:Time new roman; margin-top:1px;" class="address"> 
           '.$addr;
		$out .= '</div>';
		return $out;
	}
