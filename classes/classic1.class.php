...
class template extends commonClass {	
	...
	function printname($resume_id){
		...
		$out = '<div style="text-align:center; border-bottom:1px solid #000000; font-size:30px;" class="name">
	...
	function printaddress($resume_id){
		...
		if($res['phone_num']!="" and $res['phone_num']!="" and $addr!="")
			$addr .= '<br />';
		//Phone NUmber			
		if($res['phone_num']!=""){
		...
		if($res['street']=="" and $res['street2']=="" and $res['city']=="" and $res['state']=="" and $res['zip']=="" and $res['phone_num']=="" and $res['email']==""){
			$addr = 'Street Address, City State Zip<br />Home: Phone Number<br />Email address';
		}
		$out = '<div style="text-align:center; border-top:1px solid #000000; font-size:12px; font-family:Arial, Helvetica, sans-serif; margin-top:1px;" class="address"> 
           '.$addr;
	...
	function print_all_sections($resume_id){
		...
		foreach($res_secs as $res_sec) {
			...
			<div style="font-size:18px; margin:0px; font-weight:bold; line-height:30px;" class="section_title">'.$section['section_name'];
			...
			if($formfile=="single_value"){
				...
				$thisout .= '
				<div class="section_title_edit">
					<a class="fancybox fancybox.ajax" title="Edit Section" title="Edit Section" href="Ajax/'.$formfile.'.php?rid='.$resume_id.'">Edit</a>
					 &nbsp; |  &nbsp; 
					<a class="fancybox fancybox.ajax" title="Remove Section" href="Ajax/section_delete.php?rid='.$resume_id.'&id='.$section['sectionid'].'">Remove</a>
					 &nbsp; |  &nbsp; 
					<a class="fancybox fancybox.ajax" title="Move Section" href="Ajax/reorder_section.php?id='.$resume_id.'">Move</a>
				...
				$thisout .= '
				<div class="section_title_edit">
					<a class="fancybox fancybox.ajax" title="Add New" href="Ajax/'.$formfile.'.php?rid='.$resume_id.'">Add New</a>
					 &nbsp; |  &nbsp; 
					<a class="fancybox fancybox.ajax" title="Remove Section" href="Ajax/section_delete.php?rid='.$resume_id.'&id='.$section['sectionid'].'">Remove</a>
					 &nbsp; |  &nbsp; 
					<a class="fancybox fancybox.ajax" title="Move Section" href="Ajax/reorder_section.php?id='.$resume_id.'">Move</a>
		...
		foreach($work_value as $work) {
			...
			<div style=" overflow:hidden; " class="highlights">
				...
				<a class="fancybox fancybox.ajax" title="Edit Section" href="Ajax/editwork_history.php?id='.$work['work_his_id'].'">Edit</a>
				 &nbsp; |  &nbsp; 
				<a class="fancybox fancybox.ajax" title="Edit Section" href="Ajax/delwork_history.php?id='.$work['work_his_id'].'">Delete</a>
				</div>
				<table style="width:100%">
				<tr>
	...
	function print_executive_exp($resume_id){
		...
		<div style=" overflow:hidden; " class="highlights">
			<div class="highlights_edit">
			<a class="fancybox fancybox.ajax" href="Ajax/edit_executive_exp.php?id='.$exp['executive_exp_id'].'">Edit</a>
			 &nbsp; |  &nbsp; 
			<a class="fancybox fancybox.ajax" href="Ajax/del_executive_exp.php?id='.$exp['executive_exp_id'].'">Delete</a>
			</div>
			<table style="width:100%">
				<tr>
	...
	function print_technical_skills($resume_id){
		...
		if(count($skills)>0){
			...
			foreach($skills as $skill) {		
				$thisout .= '<div style=" overflow:hidden; " class="t_skills">
				...
				<tr class="t_skills">
		   		...
		   		<td style=" overflow:hidden; width:30%;">'.$skill['expertise'].'</td>
	...
	function viewname($resume_id){
		$res = $this->load_single_resume($resume_id);		
		$out = '<div style="text-align:center; border-bottom:1px solid #000000; font-size:30px;">
           '.$res['first_name'].' '.$res['last_name'];
		$out .= '</div>';
		return $out;
	}
	...
	function viewaddress($resume_id){
		...
		if($res['phone_num']!="" and $res['phone_num']!="")
			$addr .= '<br />';
		//Phone NUmber			
		if($res['phone_num']!=""){
			$addr .= ' Home: '.$res['phone_num'];	
		...
		$out = '<div style="text-align:center; border-top:1px solid #000000; font-size:15px; margin-top:1px;" class="address"> 
           '.$addr;
		$out .= '</div>';
		return $out;
	}
	...
	function view_education($resume_id){
		...
		foreach($edu_value as $edu){
			...
			$thisout .= '			
		<div style=" overflow:hidden; padding:10px 0" class="highlights">
			<table style="width:100%;">
			    <tr>
				   <td style="vertical-align:top; width:80%">
				   	  	<div style="font-weight:bold;">'.$edu['school'].'</div>
