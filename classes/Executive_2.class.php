...
class template extends commonClass {	
	...
	function printaddress($resume_id){
		...
		if($res['street']=="" and $res['city']=="" and $res['state']=="" and $res['zip']==""){
			$addr = 'Street Address, City State Zip <br> Home: Phone Number <br> Email address';
		}
		$out = '<div style="text-align:center; border-top:4px solid #000000; font-size:17px;  font-family:Time new roman; margin-top:1px;" class="address"> 
           '.$addr;
	...
	function print_all_sections($resume_id){
		...
		foreach($res_secs as $res_sec) {
			...
			<span style="background-color: #FFFFFF;font-size: 20px;font-weight: normal; padding:0 10px;" class="executive_head">'.$section['section_name'].'</span>';
	...
	function print_single_value($resume_id, $section_id){
		...
		return '<div style="margin-left:12%;width:75%;font-size:14px !important;">'.$row[$field].'</div>';
	}
	function print_education($resume_id){
		...
		<div style="overflow:hidden;  margin-left:12%;" class="highlights">
			...
			<table style="width:100%">
					<tr>
						<td style="vertical-align:top; width:97%;text-align:center">
							<div><b>'.$degree.'</b></div>
							</div>'.$address.'</div>
						</td>
					</tr>
	...
	function print_executive_summary($resume_id){
		...
		<div style="overflow:hidden;margin-left:12%" class="highlights">
		   <table style="width:100%">
		   		<tr>
					<td colspan="2"style="font-size:14px !important;margin-left:5%;">
	...
	function view_technical_skills($resume_id){
		...
		if(count($skills)>0){
			$thisout = '
			<table style="width:88%;margin-left:12%">
			   <tr>
