...
$out='<div style=" border-top: 2px solid black;  margin-top: 42px; " class="executive"> <span style="
    background-color: #FFFFFF;display: table;font-size: 30px;font-weight: normal;margin: auto;padding: 0 9px;
    position: relative;text-align: center;top: -20px; font-family:Time new roman;" class="executive_head">'.$res['first_name'].' '.$res['last_name'].'</span>';
	...
	function printaddress($resume_id){
		...
		if($res['street']=="" and $res['street2']=="" and $res['city']=="" and $res['state']=="" and $res['zip']=="" and $res['phone_num']==""){
			$addr = 'Street Address  City State Zip <br> Home: Phone Number Email address';
		}		
		$out = '<div style="text-align:center;font-size:15px; margin-top:1px; font-family:Time new roman;" class="address"> 
           '.$addr;
	...
	function print_all_sections($resume_id){
		...
		foreach($res_secs as $res_sec) {
			...
			$thisout .= '
			<div class="section_block" style="margin-top:42px;">
			<div style="font-family:Time new roman; background: url(\'images/moderan-title.jpg\') repeat-x scroll center center transparent;" class="section_title">
			<span style="background-color: #FFFFFF;display: table;font-size: 20px;font-weight: normal; font-weight:bold;">'.$section['section_name'].'</span>';
	...
	function print_single_value($resume_id, $section_id){
		...
		return '<div style="margin-left:20%;">'.$row[$field].'</div>';
	}
			...
			<table style="width:100%">
					<tr>
						...
						<td style="vertical-align:top; width:80%;text-align:left">
							<div><b>'.$edu['school'].'</b> '.$address.'</div>
							<div><b>'.$degree.'</b></div>
						</td>
					</tr>
					<tr>
	...
	function print_highlight($resume_id){
		...
		$thisout = '
		<div style="overflow:hidden;margin-left:20%;margin-top:10px;" class="highlights">
				...
				<table  style="width:100%">
					<tr>
						...
						<td style="vertical-align:top;text-align:left; width:80%">
							<div><b>'.$exp['company_name'].'</b> '.$address.'</div>
			...
			<div style=" overflow:hidden;" class="highlights">
				...
				<tr>
					...
					<td style="width:80%; vertical-align:top;text-align:left;">
						<div class="skills_column_1"><b>'.$work['company_name'].'</b> '.$address.'</div>
...
/*$out='<div style=" border-top: 2px solid black;  margin-top: 42px; " class="executive"> <span style="
    ...
    position: relative;text-align: center;top: -20px; font-family:Time new roman;" class="executive_head">'.$res['first_name'].' '.$res['last_name'].'</span>';
		$out .= '</div>';*/
		$length_name = (strlen($res['first_name'])+strlen($res['last_name'])+6);
		$twoside = (100-$length_name)/2;
		$out='
			<table width="100%" cellpadding="5">
				<tr>
					...
					<td width="'.$length_name.'%" style="text-align:center">
	...
	function viewaddress($resume_id){
		...
		if($res['street']=="" and $res['city']=="" and $res['state']=="" and $res['zip']=="" and $res['phone_num']=="" and $res['cell_num']==""){
			$addr = 'Street Address  City State Zip <br> Home: Phone Number Cell: Cell Phone Number Email address';
		}		
		$out = '<div style="text-align:center;font-size:15px; font-family:Time new roman; margin-top:1px;" class="address"> 
           '.$addr;
		$out .= '</div>';
		return $out;
	}
	...
	function view_all_sections($resume_id){
		...
		foreach($res_secs as $res_sec) {
			...
			<table width="100%" cellpadding="5">
				<tr>
					<td width="'.$length.'%" style="text-align:left">
						<div style="font-weight:bold;">'.$section['section_name'].'</div>
					</td>
					<td width="'.floor($side).'%">
						<hr color="#000000" />
					</td>
	...
	function view_single_value($resume_id, $section_id){
		...
		$sql = "SELECT $field FROM resume_other_section where resume_id='$resume_id'";
		$row = $this->query_first($sql);
		return '<div style="margin-left:20%;margin-top:10px;">'.$row[$field].'</div>';
	}
