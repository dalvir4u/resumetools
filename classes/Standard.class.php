...
class template extends commonClass {	
	...
	function printname($resume_id){
		...
		$out = '<div style="text-align:left; font-size:35px; font-family:Arial;" class="name">
	...
	function printaddress($resume_id){
		...
		$out = '<div style="text-align:center;font-size:14px; margin-top:1px; font-family:Arial;" class="address"> 
           '.$addr;
	...
	function print_all_sections($resume_id){
		...
		foreach($res_secs as $res_sec) {
			...
			<div style="font-size:23px; margin:0px; font-family:Arial;" class="section_title">'.$section['section_name'];
	...
	function print_experience($resume_id){
		...
		foreach($exp_value as $exp) {
			...
			if($exp['state']!="")
				$address .= $exp['state'];
			if($exp['company_name']!="")
				$comp_add .='<b>'.$exp['company_name'].'</b>';
			if($address!="" and $exp['company_name']!="")
				$comp_add .='-';
			if($address!="")
				$comp_add .=$address;
			if($exp['date_from']!="")
				...
				<table style="width:100%">
					<tr>
						<td style="vertical-align:top; width:70%">
							<div><b>'.$exp['position'].'</b> '.$date_between.'</div>
							<div>'.$comp_add.'</div>
							<div></div>
						</td>
	...
	function print_work_history($resume_id){
		...
		foreach($work_value as $work) {		
			...
			if($work['state']!="")
				$address .= $work['state'];
			if($work['position']!="")
				$pos_add .= '<b>'.$work['position'].'</b>';
			if($work['position']!="" and $address!="")
				$pos_add .= '-';
			if($address!="")
				$pos_add .= $address;
			if($work['date_from']!="")
			...
			<div style=" overflow:hidden; margin-top:10px;" class="highlights">
				...
				<tr>
					<td style="width:100%; vertical-align:top;">
						<div>'.$work['company_name'].' '.$date_between.'</div>
						<div>'.$pos_add.'</div>
					</td>
				</tr>
				</table>
	...
	function print_technical_skills($resume_id){
		...
		if(count($skills)>0){
			$thisout = '
			<table style="width:97%; margin-top:10px;">
			   <tr>
	...
	function viewaddress($resume_id){
		...
		if($res['street']=="" and $res['city']=="" and $res['state']=="" and $res['zip']=="" and $res['phone_num']=="" and $res['cell_num']=="" and $res['email']==""){
			$addr = '
			...
			<table style="width:100%">
				<tr>
					...
					<td style="width:50%; vertical-align:top;text-align:right;">
						<div>Home: Phone Number</div>
						<div>Cell: Cell Phone Number</div>
						<div>Email address</div>
					</td>						
				</tr>
			</table>';
		}
		$out = '<div style="text-align:center;font-size:14px; font-family:Arial; margin-top:1px;" class="address"> 
           '.$addr;
		$out .= '</div>';
		return $out;
	}
	...
	function view_education($resume_id){
		...
		$degree = '';
			...
			if($edu['degree']!="")
				$degree .=$edu['degree'];
			$thisout .= '			
		<div style="overflow:hidden; margin-top:20px;" class="highlights">
			<table style="width:100%;">
			    <tr>
				   <td style="vertical-align:top; width:100%">
				   	  	<div><b>'.$education.'</b></div>
